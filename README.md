# Хэш-таблица
## О проекте
В этом проекте реализована хэш-таблица с обработкой коллизий методом цепочек. В первой части проекта было проведено исследование эффективности нескольких хэш-функций. Во второй части оптимизация проводилась с использованием трех методов:
1) оптимизация AVX;
2) вставка фрагмента кода на ассемблере;
3) замена некоторой функции на собственную, которая написана на ассемблере.

## Теоретическое отступление
Для тех, кто впервые встретился с этим понятием, быстро объясню суть хэш-таблиц.

*Хэш-таблица* - это структура данных, в которой информация хранится парами: первый элемент - "ключ", второй - "значение". Поиск по хэш-таблице начинается с высчитывания значения хэш-функции от ключа. Полученное значение будет являться индексом ячейки в хэш-таблице, где лежит информация, соответствующая этому ключу. Пример, на котором всё станет ясно. Пусть есть дом, в нём 4 комнаты: "зелёная", "красная", "синяя", "серая". В "серой" комнате лежит гора алмазов, в "синей" - гора сапфиров, в "красной" - гора рубинов, в "зелёной" - гора изумрудов. Предположим, что функция, когда ей передаешь слово "зелёная", возвращает 0, когда передаёшь "красная" - возвращает 1, когда передаёшь "синяя" - возвращает 2, когда передеаёшь "серая" - возвращает 3. Эти значения и будут индексами в соответствующей хэш-таблице, которая будет иметь вид:
| 0 | 1 | 2  | 3 |
| ------------- | ------------- | ------------- | ------------- |
| "зелёная"  | "красная"  | "синяя"  | "серая"  |
| изумруды  | рубины  | сапфиры  | алмазы  |

То есть время поиска по ней будет составлять O(1) - просто назовите слово и получите всю информацию, которая ему соответствует.

## Часть 1 - исследование хэш-фукнций
### Hash function returning 1
Ох... 😬 Эта хэш-функция самая ужасная из тех, что вы увидите здесь. Она всегда возвращает 1.

```c
size_t hashRet1 (char * word, size_t length)
{
    return 1;
}
```

![Распределение хэш-функции](./imgs/alw_ret_1.png)

### Hash function returning 1-st byte
Чуть лучше, чем предыдущая, возвращает 1-ый байт слова в кодировке ASCII. Для узкого круга задач она может подойти, но всё равно безумное количество коллизий.

```c
size_t hashFirstByte (char * word, size_t length)
{
    return word[0];
}
```

![Распределение хэш-функции](./imgs/first_byte.png)

### HashLength
Возвращает длину слова. Хэш-функция так же плоха, но... она хотя бы представляет интерес в научном плане: теперь мы знаем среднюю длину слова в английском тексте :grin:

```c
size_t hashLength (char * word, size_t length)
{
    return length;
}
```

![Распределение хэш-функции](./imgs/length.png)

### HashSum
Возвращает сумму ASCII-кодов букв слова. Ситуация становится лучше.

```c
size_t hashSumAscii (char * word, size_t length)
{
    size_t sum = 0;
    for (size_t i = 0; i < length; i++)
    {
        sum += word[i];
    }

    return sum;
}
```

![Распределение хэш-функции](./imgs/sum.png)

### HashRol
Использует операцию rol. Вполне приемлемая функция, коллизий не так много по сравнению с предыдущими функциями.

```cтакая
size_t hashRol (char * word, size_t length)
{
    size_t h = 0;
    for (size_t i = 0; i < length; i++)
    {
        h = rol(h)^word[i];
    }
    return h;
}
```

![Распределение хэш-функции](./imgs/rol.png)

### HashRor
Использует операцию ror. Несколько хуже предыдущей, но использовать в некоторых задачах можно.

```c
size_t hashRor (char * word, size_t length)
{
    size_t h = 0;
    for (size_t i = 0; i < length; i++)
    {
        h = ror(h)^word[i];
    }
    return h;
}
```
![Распределение хэш-функции](./imgs/ror.png)

### HashBkdr
Реализация хэш-функции BKDR. Лучшая функция в этой подборке.

```c
size_t hashBkdr (char * word, size_t lengthWord)
{
    unsigned int seed = 131;
    unsigned int hash = 0;

    while (*word){
        hash = hash * seed + (*word++);
    }

    return (hash & 0x7FFFFFFF);
}
```
![Распределение хэш-функции](./imgs/bkdr.png)

## Вывод
Лучше всего себя показала последняя хэш-функция BKDR. Именно её из всех предложенных я бы советовал использовать при анализе текста, так как поиск по хэш-таблице, полученной с её использованием, будет достаточно быстрым.


## Часть 2 - оптимизация

### Версия 0
Удалены все хэш-функции кроме hashBkdr, которую мы взяли за основную, убраны все отладочные функции, добавлен стресс-тест. Для того чтобы время, необходимое на системные вызовы, составляло малую часть от работы функции htFind, будем увеличивать количество итераций в цикле до тех пор, пока то, во сколько раз мы это увеличим их, не сравнялось с количеством разов, в которые увеличивается время выполнения программы. Так, мы получим 100000000 оборотов. Время работы программы равно 17.328 секундам. Обратимся к профилировщику для того, чтобы понять, какие части программы являются наиболее "горячими".

![Версия 0](./imgs/version0.png)

Видно, что функция hashBkdr тратит больше всего ресурсов. Оптимизируем её. Для этого напишем её на ассемблере. Заметьте, что это нарушит переносимость программы, однако может ускорить программу на этой машине.

### Версия 1
Функция hashBkdr заменена на ассемблерный аналог. Время работы программы в среднем составляет 16.092 секунды. Относительный прирост производительности равен 17.328/16.092.

Ассемблерный аналог функции hashBkdr:
```asm
hashBkdr:
   push rdx
   push rbx
   push rcx


   xor rax, rax             ; hash
   mov rbx, 131             ; coefficient
   xor rcx, rcx
.loop:
   movzx rcx, byte [rdi]    ; word

   test rcx, rcx
   jz .end

   mul rbx

   lea rax, [rax + rcx]
   inc rdi
   jmp .loop

.end:
   and eax, 0x7fffffff

   pop rcx
   pop rbx
   pop rdx
   ret
```

Снова посмотрим на данные профилировщика:

![Версия 1](./imgs/version1.png)

Видно, что `__libc_start_call_main` требует больше всего ресурсов, однако это системная функция и оптимизировать её мы не сможем. Попробуем оптимизировать `strlen` и `memcmp`. Видно, что компилятор использовал avx2-аналоги этих фукнций, но, быть может, мы справимся лучше.

### Версия 2
Для того чтобы работать avx2 инструкциями, каждое слово будем хранить в области размером 256 бит. Это будет требовать от компьютера больше памяти, однако это позволит ускорить работу программы. Также не забудем выровнять ячейки памяти, где хранятся слова, по 32-байтной границе. Поскольку размер массива для слова постоянен, постольку мы можем избавиться от `strlen`.

memcmpAvx:
```c
static int memcmpAvx (const void * str1, const void * str2, size_t length)
{
    __m256i ar1 = _mm256_loadu_si256 ((__m256i *) str1);
    __m256i ar2 = _mm256_loadu_si256 ((__m256i *) str2);

    __m256i cmp = _mm256_cmpeq_epi8 (ar1, ar2);
    int mask = _mm256_movemask_epi8 (cmp);

    if (mask == 0xffffffff)
    {
        return 0;
    }
    return 1;
}
```

memcmpAsm:
```c
inline static int memcmpAsm (const void * str1, const void * str2, size_t length)
{
    int result = 0;

    asm
    (
        ".intel_syntax noprefix\n\t"
        "cld\t\n"
        "repe cmpsb\t\n"
        "je .eq\t\n"
        ".noeq:\t\n"
        "dec %1\t\n"
        "dec %2\t\n"
        "mov %0, [%1]\t\n"
        "sub %0, [%2]\t\n"
        "jmp .end\t\n"
        ".eq:\t\n"
        "mov %0, 0\t\n"
        ".end:\t\n"
        ".att_syntax\t\n"
        : "=a" (result)
        : "S" (str1), "D" (str2), "c" (length)
    );

    return result;
}
```

Измерения показали, что наша реализация memcmp при помощи AVX и с уступает `__memcmp_avx2_movbe`.

|__memcmp_avx2_movbe| memcmpAvx |   memcmpAsm   |
| ------------- | ------------- | ------------- |
|     15.561    |     17.026    |     17.908    |

Посмотрим на результат профилировщика:

![Версия 2](./imgs/version2.png)

Дополнительно к нашим оптимизациям скопмилируем проект c помощью флага -O3 и измерим время выполнения.
Результат: 14.932 секунд.

Запустим версию проекта без ручных оптимизаций с флагом -O3.
Результат: 17.250 секунд.

На этом мы заканчиваем наш эксперимент.

Вывод
Общий прирост скорости составляет 86% по тактам. Рассчитаем коэффициент оправданности наших действий по формуле:

$\frac{\text{acceleration value}}{\text{number of assembly lines}} \cdot 1000$.

Не будем учитывать имена меток, названия функций и комментарии. Таким образом, получим:

$\frac{1.86}{18} \cdot 1000 = 103.33$
 
